...
/* v75: stable rebuild from v69. Add/edit popup integrated without v70-v74 overrides. */
function Header(){
  const nav=[['dashboard','今日'],['schedule','予定'],['staff','担当'],['changes','変更登録'],['next','翌月作成'],['masters','マスタ'],['print','印刷'],['settings','設定']];
  return `<div class="topbar"><div class="topbar-inner"><div class="brand"><h1>送迎管理</h1><small>本番安定版 1.8.0 ／ ${fiscalLabel(state.fiscalYear)} ／ 最終更新：${esc(state.lastLoaded||'未取得')} ／ 自動更新：${state.autoRefresh?'ON':'OFF'}</small></div><div class="top-actions no-print"><button id="reloadBtn" class="primary">更新</button><button id="autoBtn">自動更新 ${state.autoRefresh?'ON':'OFF'}</button></div></div><div class="mobile-view-switch no-print"><label>画面<select id="mobileViewSelect">${nav.map(([k,l])=>`<option value="${k}" ${state.view===k?'selected':''}>${l}</option>`).join('')}</select></label></div><div class="nav no-print">${nav.map(([k,l])=>`<button data-view="${k}" class="${state.view===k?'active':''}">${l}</button>`).join('')}</div></div>`;
}
function eventTypeKey(type){ return type==='送り' ? 'dropoff' : 'pickup'; }
function typeLabel(type){ return type==='dropoff' ? '送り' : type==='pickup' ? '迎え' : type; }
function getScheduleFromFiscal(id){
  const fy=state.fiscalData;
  if(!fy || !id) return null;
  for(const [month,pack] of Object.entries(fy.months||{})){
    const item=(pack.schedules||[]).find(s=>s.id===id);
    if(item) return {month,item:normalizeSchedule(item),pack};
  }
  const cur=currentData().schedules.find(s=>s.id===id);
  return cur ? {month:(cur.date||state.month).slice(0,7),item:normalizeSchedule(cur),pack:null} : null;
}
function upsertScheduleEverywhere(item, oldMonth){
  item=normalizeSchedule(item);
  const newMonth=(item.date||state.month+'-01').slice(0,7);
  if(!state.fiscalData) state.fiscalData=normalizeFiscalData({fiscalYear:state.fiscalYear,months:{},masters:currentData().masters});
  if(oldMonth && oldMonth!==newMonth && state.fiscalData.months?.[oldMonth]){
    state.fiscalData.months[oldMonth].schedules=(state.fiscalData.months[oldMonth].schedules||[]).filter(s=>s.id!==item.id);
  }
  const pack=ensureFiscalMonth(state.fiscalData,newMonth);
  const idx=(pack.schedules||[]).findIndex(s=>s.id===item.id);
  if(idx>=0) pack.schedules[idx]=item; else pack.schedules.push(item);
  if(newMonth===state.month){
    const d=currentData();
    const i=d.schedules.findIndex(s=>s.id===item.id);
    if(i>=0) d.schedules[i]=item; else d.schedules.push(item);
    state.data=d;
  }
  if(oldMonth===state.month && oldMonth!==newMonth){
    const d=currentData(); d.schedules=d.schedules.filter(s=>s.id!==item.id); state.data=d;
  }
  state.dirty=true;
}
async function saveFiscalAfterPopup(message){
  state.loading=true; render();
  try{
    await api.saveFiscalData(state.fiscalYear,state.fiscalData,state.fiscalData?.meta?.version);
    const fresh=await api.getFiscalData(state.fiscalYear);
    ...
    state.lastLoaded=new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'});
    writeCache(state.fiscalYear,state.fiscalData);
    showNotice(message||'保存しました','年度データに保存しました。');
  }catch(e){ showNotice('保存できませんでした',e.message,'bad'); }
  finally{ state.loading=false; render(); }
}
function scheduleEvents(rows){
  ...
  rows.filter(activeSchedule).forEach(s=>{
    ...
    if(ns.pickupTime || ns.pickupPlace || ns.pickupStaff) items.push({id:ns.id,date:ns.date,type:'迎え',kind:'pickup',time:ns.pickupTime||'',child:ns.child||'',place:ns.pickupPlace||'',staff:ns.pickupStaff||'',note:ns.note||'',status:ns.status||'予定'});
    if(ns.dropoffTime || ns.dropoffPlace || ns.dropoffStaff) items.push({id:ns.id,date:ns.date,type:'送り',kind:'dropoff',time:ns.dropoffTime||'',child:ns.child||'',place:ns.dropoffPlace||'',staff:ns.dropoffStaff||'',note:ns.note||'',status:ns.status||'予定'});
  });
  return items.sort((a,b)=>(a.date||'').localeCompare(b.date||'')||(a.time||'').localeCompare(b.time||'')||a.type.localeCompare(b.type,'ja')||(a.child||'').localeCompare(b.child||'','ja'));
}
function editButton(id,type){ return `<button type="button" class="edit-icon no-print" data-edit-schedule="${esc(id||'')}" data-edit-type="${esc(eventTypeKey(type))}" aria-label="編集">編集</button>`; }
function addButton(date,type,label){ return `<button type="button" class="add-inline no-print" data-add-popup="${esc(date)}" data-add-type="${esc(eventTypeKey(type))}">${esc(label||'＋追加')}</button>`; }
function compactEventCard(e){
  const cls=e.type==='迎え'?'pickup':'dropoff';
  return `<div class="cal-event ${cls}" title="${esc(`${e.type} ${e.time} ${e.child} ${e.place} ${e.staff}`)}"><div class="cal-row-top">${editButton(e.id,e.type)}<div class="cal-time">${esc(e.time||'時刻なし')}</div></div><div class="cal-main"><span class="type">${esc(e.type)}</span> ${esc(e.child||'未入力')}</div><div class="cal-sub">${esc(e.place||'場所なし')} ／ ${esc(e.staff||'担当なし')}</div>${e.note?`<div class="cal-note">${esc(e.note)}</div>`:''}</div>`;
}
function routeTables(rows,title){
  const date=(rows[0]?.date)||state.date;
  const pickup=rows.filter(activeSchedule).filter(s=>s.pickupTime||s.pickupPlace||s.pickupStaff).sort((a,b)=>(a.pickupTime||'').localeCompare(b.pickupTime||''));
  const drop=rows.filter(activeSchedule).filter(s=>s.dropoffTime||s.dropoffPlace||s.dropoffStaff).sort((a,b)=>(a.dropoffTime||'').localeCompare(b.dropoffTime||''));
  const table=(type,items)=>`<div class="panel"><div class="panel-head"><div class="panel-title"><h2>${type}</h2><p>${title}</p></div>${state.view==='schedule'?addButton(date,type,`＋ ${type}追加`):''}</div><div class="table-wrap"><table><thead><tr><th class="edit-col">編集</th><th>時間</th><th>児童名</th><th>場所</th><th>担当</th><th>備考</th></tr></thead><tbody>${items.map(s=>`<tr><td>${editButton(s.id,type)}</td><td>${esc(type==='迎え'?s.pickupTime:s.dropoffTime)}</td><td>${esc(s.child)}</td><td>${esc(type==='迎え'?s.pickupPlace:s.dropoffPlace)}</td><td>${esc(type==='迎え'?s.pickupStaff:s.dropoffStaff)}</td><td>${esc(s.note)}</td></tr>`).join('')||'<tr><td colspan="6">予定なし</td></tr>'}</tbody></table></div></div>`;
  return `<div class="split">${table('迎え',pickup)}${table('送り',drop)}</div>`;
}
function WeekView(){
  const days=weekDatesFor(state.date);
  ...
  const title=`${jpDate(days[0])} 〜 ${jpDate(days[6])}`;
  const allEvents=scheduleEvents(days.flatMap(day=>rowsForDateAnyMonth(day)));
  return `<section class="panel"><div class="panel-head"><div class="panel-title"><h2>週表示</h2><p>${title}${months.length>1?` ／ ${months.join('・')} の予定を表示`:''}</p></div><span class="badge">${allEvents.length}件</span></div><div class="week-calendar"><div class="week-head-row">${days.map(day=>`<button data-day="${day}" class="week-head ${day===state.date?'selected':''}"><strong>${jpDate(day)}</strong><span>${scheduleEvents(rowsForDateAnyMonth(day)).length}件</span></button>`).join('')}</div><div class="week-body">${days.map(day=>{const events=scheduleEvents(rowsForDateAnyMonth(day)); return `<div class="week-col ${day===state.date?'selected':''}"><div class="week-add-row">${addButton(day,'迎え','＋追加')}</div>${events.map(compactEventCard).join('')||'<div class="empty-mini">予定なし</div>'}</div>`;}).join('')}</div></div></section><section class="panel"><div class="panel-head"><div class="panel-title"><h2>週一覧</h2><p>同じ週の予定を一覧で確認できます。</p></div></div>${eventListTable(allEvents)}</section>`;
}
function eventListTable(events){
  return `<div class="table-wrap"><table class="compact-table"><thead><tr><th class="edit-col">編集</th><th>日付</th><th>区分</th><th>時間</th><th>児童名</th><th>場所</th><th>担当</th><th>備考</th></tr></thead><tbody>${events.map(e=>`<tr><td>${editButton(e.id,e.type)}</td><td>${esc(jpDate(e.date))}</td><td>${esc(e.type)}</td><td>${esc(e.time)}</td><td>${esc(e.child)}</td><td>${esc(e.place)}</td><td>${esc(e.staff)}</td><td>${esc(e.note)}</td></tr>`).join('')||'<tr><td colspan="8">予定なし</td></tr>'}</tbody></table></div>`;
}
function StaffDetailTable(rows){
  return `<div class="table-wrap"><table id="staffDetailTable" class="compact-table"><thead><tr><th class="edit-col">編集</th><th>日付</th><th>区分</th><th>時間</th><th>児童名</th><th>場所</th><th>担当</th><th>状態</th><th>備考</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${editButton(r.id,r.type)}</td><td>${esc(jpDate(r.date))}</td><td><span class="badge ${r.type==='迎え'?'ok':'warn'}">${esc(r.type)}</span></td><td>${esc(r.time)}</td><td>${esc(r.child)}</td><td>${esc(r.place)}</td><td>${esc(r.staff)}</td><td>${esc(r.status)}</td><td>${esc(r.note)}</td></tr>`).join('')||'<tr><td colspan="9">担当予定はありません。</td></tr>'}</tbody></table></div>`;
}
function staffEventsForRows(rows, staffName){
  const list=[];
  rows.filter(activeSchedule).forEach(s=>{
    const ns=normalizeSchedule(s);
    if((!staffName || ns.pickupStaff===staffName) && (ns.pickupTime || ns.pickupPlace || ns.pickupStaff)) list.push({id:ns.id,date:ns.date,type:'迎え',kind:'pickup',time:ns.pickupTime||'',child:ns.child||'',place:ns.pickupPlace||'',staff:ns.pickupStaff||'',note:ns.note||'',status:ns.status||'予定'});
    if((!staffName || ns.dropoffStaff===staffName) && (ns.dropoffTime || ns.dropoffPlace || ns.dropoffStaff)) list.push({id:ns.id,date:ns.date,type:'送り',kind:'dropoff',time:ns.dropoffTime||'',child:ns.child||'',place:ns.dropoffPlace||'',staff:ns.dropoffStaff||'',note:ns.note||'',status:ns.status||'予定'});
  });
  return list.filter(x=>x.staff).sort((a,b)=>(a.date||'').localeCompare(b.date||'')||(a.time||'').localeCompare(b.time||'')||a.type.localeCompare(b.type,'ja')||(a.child||'').localeCompare(b.child||'','ja'));
}
function showSchedulePopup({id='',type='pickup',date=state.date}={}){
  const found=id?getScheduleFromFiscal(id):null;
  const item=found?.item || {id:uid('sch'),date,child:'',childId:'',status:'予定',pickupTime:'',pickupPlace:'',pickupStaff:'',dropoffTime:'',dropoffPlace:'',dropoffStaff:'',note:''};
  const kind=type||'pickup';
  const isPickup=kind==='pickup';
  const time=isPickup?item.pickupTime:item.dropoffTime;
  const place=isPickup?item.pickupPlace:item.dropoffPlace;
  const staff=isPickup?item.pickupStaff:item.dropoffStaff;
  document.querySelector('.modal-backdrop')?.remove();
  const modal=document.createElement('div');
  modal.className='modal-backdrop';
  modal.innerHTML=`<div class="modal-card" role="dialog" aria-modal="true"><div class="modal-head"><h2>${id?'予定を変更':'予定を追加'}</h2><button type="button" class="modal-close" data-modal-close>閉じる</button></div><div class="modal-grid"><label class="field">日付<input id="modalDate" type="date" value="${esc(item.date||date)}"></label><label class="field">区分<select id="modalType"><option value="pickup" ${kind==='pickup'?'selected':''}>迎え</option><option value="dropoff" ${kind==='dropoff'?'selected':''}>送り</option></select></label><label class="field">状態<select id="modalStatus"><option ${item.status==='予定'?'selected':''}>予定</option><option ${item.status==='イベント'?'selected':''}>イベント</option><option ${item.status==='欠席'?'selected':''}>欠席</option></select></label><label class="field">児童名<select id="modalChild">${options(childNames(),item.child,'児童名')}</select></label><label class="field">時間<input id="modalTime" type="time" value="${esc(time)}"></label><label class="field">場所<input id="modalPlace" list="modalPlaces" value="${esc(place)}"></label><label class="field">担当<select id="modalStaff">${options(staffNames(),staff,'担当者')}</select></label><label class="field modal-note">備考<input id="modalNote" value="${esc(item.note||'')}"></label></div><datalist id="modalPlaces">${placeNames(isPickup?'pickup':'dropoff').map(p=>`<option value="${esc(p)}">`).join('')}</datalist><div class="modal-actions"><button type="button" data-modal-close>キャンセル</button><button type="button" class="primary" id="modalSaveSchedule">登録変更</button></div></div>`;
  document.body.appendChild(modal);
  modal.querySelectorAll('[data-modal-close]').forEach(b=>b.addEventListener('click',()=>modal.remove()));
  modal.addEventListener('click',e=>{ if(e.target===modal) modal.remove(); });
  modal.querySelector('#modalType')?.addEventListener('change',e=>{
    const next=e.target.value; const pickup=next==='pickup';
    modal.querySelector('#modalTime').value=pickup?(item.pickupTime||''):(item.dropoffTime||'');
    modal.querySelector('#modalPlace').value=pickup?(item.pickupPlace||''):(item.dropoffPlace||'');
    modal.querySelector('#modalStaff').value=pickup?(item.pickupStaff||''):(item.dropoffStaff||'');
  });
  modal.querySelector('#modalSaveSchedule')?.addEventListener('click',async()=>{
    const oldMonth=found?.month || (item.date||date).slice(0,7);
    const next={...item};
    next.date=modal.querySelector('#modalDate').value;
    next.status=modal.querySelector('#modalStatus').value;
    next.child=modal.querySelector('#modalChild').value;
    const child=currentData().masters.children.find(c=>c.displayName===next.child); next.childId=child?.id||next.childId||'';
    next.note=modal.querySelector('#modalNote').value;
    const selectedType=modal.querySelector('#modalType').value;
    if(selectedType==='pickup'){
      next.pickupTime=modal.querySelector('#modalTime').value;
      next.pickupPlace=modal.querySelector('#modalPlace').value;
      next.pickupStaff=modal.querySelector('#modalStaff').value;
    }else{
      next.dropoffTime=modal.querySelector('#modalTime').value;
      next.dropoffPlace=modal.querySelector('#modalPlace').value;
      next.dropoffStaff=modal.querySelector('#modalStaff').value;
    }
    if(!next.date || !next.child){ showNotice('入力不足です','日付と児童名を入力してください。','bad'); return; }
    upsertScheduleEverywhere(next,oldMonth);
    modal.remove();
    await saveFiscalAfterPopup(id?'変更しました':'追加しました');
  });
}
const bindV69Stable = bind;
function bind(){
  bindV69Stable();
  document.querySelectorAll('[data-edit-schedule]').forEach(b=>{
    b.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); showSchedulePopup({id:b.dataset.editSchedule,type:b.dataset.editType||'pickup'}); });
  });
  document.querySelectorAll('[data-add-popup]').forEach(b=>{
    b.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); showSchedulePopup({date:b.dataset.addPopup,type:b.dataset.addType||'pickup'}); });
  });
}
